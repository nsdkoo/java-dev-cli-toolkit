...
P2: TUI with Lanterna 鈫?basic notes table, shortcut key 'q' to exit.
...
Write a Java CLI app named `DevCliToolkit note` using picocli.
...
4. `add` command:
   - Usage: `DevCliToolkit note add "Fix NullPointer in UserService" --tag=bug --status=todo`
...
6. `update` command:
   - Usage: `DevCliToolkit note update <id> --status=done --content="Refactored"`
   - Updates fields of an existing note.
7. `delete` command:
   - Usage: `DevCliToolkit note delete <id>`
   - Removes note by id.
